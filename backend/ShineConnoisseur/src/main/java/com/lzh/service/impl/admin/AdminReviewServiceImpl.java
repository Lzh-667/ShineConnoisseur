...
public class AdminReviewServiceImpl implements IAdminReviewService {
    ...
    @Override
    public Result listReviews(Integer current) {
    ...
    public Result updateReviewStatus(Long id) {
        ...
        if(SystemConstants.REVIEW_STATUS_NORMAL.equals(status)){
            ...
            success = movieService.update()
                    ...
                    .update();
            if (!success) {
                throw new BusinessException("封禁影评：更新电影评分失败");
            }
            success = userService.update()
                    ...
                    .update();
            if (!success) {
                throw new BusinessException("封禁影评：更新用户影评数失败");
            }
            // 删除缓存
            ...
            stringRedisTemplate.delete(RedisConstants.MOVIE_INFO_KEY + review.getMovieId());
            try { reviewSearchRepository.deleteById(id); } catch (Exception e) { log.error("ES 删除影评文档失败: reviewId={}", id, e); }
            ...
            success = movieService.update()
                    ...
                    .update();
            if (!success) {
                throw new BusinessException("解封影评：更新电影评分失败");
            }
            success = userService.update()
                    .setSql("review_count = review_count + 1")
                    .eq("id", review.getUserId())
                    .update();
            if (!success) {
                throw new BusinessException("解封影评：更新用户影评数失败");
            }
            stringRedisTemplate.delete(RedisConstants.MOVIE_INFO_KEY + review.getMovieId());
            review.setStatus(SystemConstants.REVIEW_STATUS_NORMAL);
            try {
                ReviewDocument doc = BeanUtil.copyProperties(review, com.lzh.document.ReviewDocument.class);
                ...
                log.error("ES 同步影评文档失败: reviewId={}", id, e);
            }
