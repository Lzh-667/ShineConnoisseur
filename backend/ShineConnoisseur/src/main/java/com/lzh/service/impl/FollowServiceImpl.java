...
public class FollowServiceImpl extends ServiceImpl<FollowMapper, UserFollow> implements IFollowService {
    ...
    public Result getFollowerList(Integer current) {
        ...
        if (pageResult.getTotal() > 0) {
            ...
            stringRedisTemplate.expire(key, RedisConstants.FOLLOWER_TTL + RandomUtil.randomInt(10), TimeUnit.MINUTES);
    ...
    public Result getFollowingList(Integer current) {
        ...
        if (pageResult.getTotal() > 0) {
            ...
            stringRedisTemplate.expire(key, RedisConstants.FOLLOWING_TTL + RandomUtil.randomInt(10), TimeUnit.MINUTES);
        ...
        try {
            ...
            if (total != null && total > 0) {
                ...
                List<Long> ids = idSet.stream()
                        .map(Long::valueOf)
                        .toList();
                return Result.ok(new PageResult<>(total, getUserDTOS(ids)));
            }
        } catch (Exception e) {
            // redis格式不兼容，删掉走DB
    ...
    public Result follow(Long id, Boolean isFollow) {
        ...
        if (isFollow) {
            ...
            if (isSuccess) {
                ...
                stringRedisTemplate.expire(RedisConstants.FOLLOWER_KEY + id, RedisConstants.FOLLOWER_TTL + RandomUtil.randomInt(10), TimeUnit.MINUTES);
                stringRedisTemplate.opsForZSet().add(RedisConstants.FOLLOWING_KEY + userId, id.toString(), now);
                stringRedisTemplate.expire(RedisConstants.FOLLOWING_KEY + userId, RedisConstants.FOLLOWING_TTL + RandomUtil.randomInt(10), TimeUnit.MINUTES);
                // 发送关注消息（事务提交后）
                ...
                dto.setContent("用户" + userService.getById(userId).getNickname() + "关注了你");
                TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                    ...
                    public void afterCommit() {
                        try {
                            ...
                            log.error("发送关注消息失败: userId={}, targetUserId={}", userId, id, e);
                        }
                    }
                });
            }
