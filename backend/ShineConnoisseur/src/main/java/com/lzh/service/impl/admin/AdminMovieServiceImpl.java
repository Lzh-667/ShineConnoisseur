...
public class AdminMovieServiceImpl implements IAdminMovieService {
    ...
    @Override
    public Result listMovies(Integer current) {
        //1.查询电影列表
    ...
    public Result updateMovie(AdminMovieDTO movieDTO,Long movieId) {
        ...
        Movie updated = movieService.getById(movieId);
        if (updated != null) {
            syncES(updated);
        }
       ...
       Movie movie = movieService.getById(id);
        ...
        if(SystemConstants.MOVIE_STATUS_NORMAL.equals(status)){
            ...
            try {
                ...
                log.error("ES 删除电影文档失败: movieId={}", id, e);
            }
            ...
            try {
                ...
                log.error("ES 同步电影文档失败: movieId={}", id, e);
            }
    ...
    private void syncES(Movie movie) {
        try {
            MovieDocument doc = BeanUtil.copyProperties(movie, MovieDocument.class);
            movieSearchRepository.save(doc);
        } catch (Exception e) {
            log.error("ES 同步电影文档失败: movieId={}", movie.getId(), e);
        }
    }
}
