...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result updateProfile(UpdateProfileDTO dto) {
        ...
        String token = stringRedisTemplate.opsForValue().get(RedisConstants.USER_TOKEN_KEY + userId);
    ...
    public Result sendResetCode(String phone) {
        ...
        String codeKey = RedisConstants.RESET_CODE_KEY + phone;
        String existCode = stringRedisTemplate.opsForValue().get(codeKey);
        if(existCode != null){
            return Result.fail("发送过于频繁，请稍后再试");
        }
        ...
        String code = RandomUtil.randomNumbers(6);
        stringRedisTemplate.opsForValue().set(codeKey, code, RedisConstants.RESET_CODE_TTL, TimeUnit.MINUTES);
        log.info("发送重置密码验证码成功：{}", code);
        return Result.ok();
    }
    @Override
    public Result resetPassword(ResetPasswordDTO dto) {
        ...
        if(!StrUtil.equals(newPassword, dto.getConfirmPassword())){
            return Result.fail("两次输入密码不一致");
        }
        // 检查错误次数
        String errorKey = RedisConstants.RESET_USER_ERR_KEY + phone;
        String errorCount = stringRedisTemplate.opsForValue().get(errorKey);
        if(StrUtil.isNotBlank(errorCount) && Integer.parseInt(errorCount) >= 5){
            return Result.fail("错误次数过多，请稍后再试");
        }
        ...
        if(!StrUtil.equals(code, cacheCode)){
            Long count = stringRedisTemplate.opsForValue().increment(errorKey);
            if(Long.valueOf(1L).equals(count)){
                stringRedisTemplate.expire(errorKey, RedisConstants.RESET_USER_ERR_TTL, TimeUnit.MINUTES);
            }
            return Result.fail("验证码错误");
        }
        stringRedisTemplate.delete(codeKey);
        ...
        // 清除该用户的登录态，强制重新登录
        String userTokenKey = RedisConstants.USER_TOKEN_KEY + user.getId();
    ...
    public Result deleteAccount() {
        ...
        boolean updated = lambdaUpdate()
                .eq(User::getId, userId)
                .set(User::getStatus, SystemConstants.USER_STATUS_DELETED)
                .update();
        if (!updated) {
            return Result.fail("注销失败");
        }
        // 清除登录态
        String userTokenKey = RedisConstants.USER_TOKEN_KEY + userId;
        String token = stringRedisTemplate.opsForValue().get(userTokenKey);
        if (StrUtil.isNotBlank(token)) {
            stringRedisTemplate.delete(RedisConstants.LOGIN_USER_KEY + token);
            stringRedisTemplate.delete(userTokenKey);
    ...
    private String createToken(User user) {
        ...
        //1.4.设置token有效期
        stringRedisTemplate.expire(tokenKey,RedisConstants.LOGIN_USER_TTL,TimeUnit.MINUTES);
        //1.5.存储反向映射：userId→token，用于封禁时清除登录态
        String userTokenKey = RedisConstants.USER_TOKEN_KEY + user.getId();
        stringRedisTemplate.opsForValue().set(userTokenKey, token, RedisConstants.LOGIN_USER_TTL, TimeUnit.MINUTES);
        return token;
    }
